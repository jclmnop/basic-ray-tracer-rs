...
mod tests {
    ...
    fn it_works() {
        ...
        let colour2 = PixelColour::new(26, 179, 249);
        let colour3 = PixelColour::new(26, 179, 249);
        let material = Material::new(
            colour.to_light_colour() * 0.2,
        ...
        for _ in 0..TEST_FRAMES {
            let render_time = timeit!({ render(&mut img, &test_camera, &test_shapes) }).as_millis();
            render_times.push(render_time);
        }
        let render_time_avg: u128 = render_times.iter().sum::<u128>() / render_times.len() as u128;
        let write_time = timeit!({ write_img(&img, &Path::new(TEST_PATH)) }).as_millis();
        println!(
            "\
